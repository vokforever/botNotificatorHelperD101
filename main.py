...
**ВАЖНО: Если сообщение содержит несколько доменов или сервисов в табличном формате - НЕ используй add_service!**
Такие сообщения должны обрабатываться специальным мульти-доменным парсером.

Правила:
1. Всегда пытайся понять намерение пользователя
2. Если нужно добавить ОДИН сервис - используй функцию add_service
3. Если нужно показать список - используй функцию list_services
4. Если нужно продлить сервис - используй функцию extend_service
...
8. **Если сообщение содержит таблицу с несколькими доменами - отвечай текстом, что это мульти-доменное сообщение**

Примеры запросов:
- "Добавь Netflix за 299 рублей до конца года" → add_service
- "Покажи все сервисы для проекта ВЛАДОГРАД" → list_services
- "Продли GitHub Pro на 3 месяца" → extend_service
- "Сколько стоит продлить домен прогрэсс.рф?" → extend_service

**НЕ используй add_service для:**
- Таблиц с несколькими доменами
- Сообщений типа "добавь в проект ВЛАДОГРАД: [список доменов]"
- Мульти-доменных сообщений
"""
...
async def handle_natural_language(update: Update, context: CallbackContext):
    ...
    try:
        ...
        # Проверяем, не является ли это мульти-доменным сообщением
        # Если да, то не используем Function Calling, а обрабатываем через multi-domain parser
        if any(keyword in text.lower() for keyword in ['добавь в проект', 'добавить в проект']) and any(keyword in text.lower() for keyword in ['.рф', '.ru', '.рус', '.com']):
            # Это мульти-доменное сообщение, обрабатываем через multi-domain parser
            await update.message.reply_text(
                "🔍 **Обнаружено мульти-доменное сообщение!**\n\n"
    ...
    if query.data == "cancel_parsed":
        ...
        try:
            ...
            if parsed_data.get('parsing_method') in ['multi_domain_parser', 'groq_ai']:
                ...
                try:
                    await query.edit_message_text(success_message, parse_mode=None)
                    print(f"✅ DEBUG: Мульти-доменные данные успешно сохранены в БД")
                except Exception as e:
                    print(f"❌ DEBUG: Ошибка при отправке сообщения: {e}")
                    # Пробуем отправить без форматирования
                    try:
                        simple_message = f"✅ Мульти-доменные данные успешно сохранены! Проект: {project}, Доменов: {len(domains)}, Сохранено: {saved_count}"
                        await query.edit_message_text(simple_message, parse_mode=None)
                    except Exception as e2:
                        print(f"❌ DEBUG: Критическая ошибка при отправке сообщения: {e2}")
                        await query.edit_message_text("✅ Данные успешно сохранены!")
                return
